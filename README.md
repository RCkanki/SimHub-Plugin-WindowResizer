...
- **Out-of-bounds profile values** — When a target rectangle exceeds the current desktop display area, WindowResizer clamps position and shrinks size as needed so the window stays fully visible within the virtual desktop bounds (including multi-monitor spans).  
